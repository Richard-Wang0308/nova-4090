...
class HybridMoleculeGenerator:
    ...
    desired_count: int = 100
        ...
        try:
            ...
            # Determine search strategy based on score thresholds
            has_very_high_score = current_max_score is not None and current_max_score > 0.18
            has_high_score = current_max_score is not None and 0.16 <= current_max_score <= 0.18
            
            # Define hierarchical search strategy with 5 tiers
            if has_very_high_score:
                ...
                search_configs = [
                    {'tier': 'Tier 1 (Ultra-tight)', 'n_mols': 1, 'budget': 10, 'min_sim': 0.90, 'n_per': 10},
                    {'tier': 'Tier 2 (Very-tight)', 'n_mols': 5, 'budget': 20, 'min_sim': 0.80, 'n_per': 4},
                    {'tier': 'Tier 3 (Tight)', 'n_mols': 10, 'budget': 30, 'min_sim': 0.70, 'n_per': 3},
                    {'tier': 'Tier 4 (Medium)', 'n_mols': 20, 'budget': 50, 'min_sim': 0.60, 'n_per': 3},
                    {'tier': 'Tier 5 (Broad)', 'n_mols': 50, 'budget': 100, 'min_sim': 0.50, 'n_per': 2},
                ]
            elif has_high_score:
                print(
                ...
                search_configs = [
                    {'tier': 'Tier 1 (Very-tight)', 'n_mols': 1, 'budget': 10, 'min_sim': 0.90, 'n_per': 10},
                    {'tier': 'Tier 2 (Tight)', 'n_mols': 5, 'budget': 20, 'min_sim': 0.80, 'n_per': 4},
                    {'tier': 'Tier 3 (Medium)', 'n_mols': 10, 'budget': 30, 'min_sim': 0.70, 'n_per': 3},
                    {'tier': 'Tier 4 (Medium-broad)', 'n_mols': 20, 'budget': 50, 'min_sim': 0.60, 'n_per': 3},
                    {'tier': 'Tier 5 (Broad)', 'n_mols': 50, 'budget': 100, 'min_sim': 0.50, 'n_per': 2},
                ]
            else:
                print(
                    f"🎯 Standard score ({current_max_score:.6f} < 0.16), "
                    f"using BROAD hierarchical strategy"
                )
                search_configs = [
                    {'tier': 'Tier 1 (Tight)', 'n_mols': 1, 'budget': 10, 'min_sim': 0.90, 'n_per': 10},
                    {'tier': 'Tier 2 (Medium-tight)', 'n_mols': 5, 'budget': 20, 'min_sim': 0.80, 'n_per': 4},
                    {'tier': 'Tier 3 (Medium)', 'n_mols': 10, 'budget': 30, 'min_sim': 0.70, 'n_per': 3},
                    {'tier': 'Tier 4 (Medium-broad)', 'n_mols': 20, 'budget': 50, 'min_sim': 0.60, 'n_per': 3},
                    {'tier': 'Tier 5 (Broad)', 'n_mols': 50, 'budget': 100, 'min_sim': 0.50, 'n_per': 2},
                ]
...
def write_scores_to_db(molecules: List[Dict[str, Any]], db_path: str = None) -> None:
    ...
    try:
        ...
        if to_insert:
            cursor.executemany(
                "INSERT OR REPLACE INTO scored_molecules (molecule_name, score, scored_at) VALUES (?, ?, CURRENT_TIMESTAMP)",
                to_insert
            )
            conn.commit()
...
async def load_submissions_from_csv(
    ...
    """
    Load submissions from both CSV and database, merge them, and return top 500.
    ...
    Returns:
        DataFrame with top 500 molecules
    """
    try:
        ...
        top_500 = molecules_df.head(500)
        
        print(f"✅ Selected top 500 molecules from combined CSV and database")
        
        if len(top_500) > 0 and 'score' in top_500.columns:
            scores = top_500['score'].dropna()
            if len(scores) > 0:
                print(
                    f"   Top 500 score range: {scores.min():.6f} to {scores.max():.6f} "
                    f"(top 3: {scores.head(3).tolist()})"
                )
...
async def generate_unique_molecules_from_top200(
    ...
    Args:
        state: State dictionary
        top_200_df: DataFrame with top 500 molecules (used as seed pool)
        hybrid_generator: Initialized HybridMoleculeGenerator instance
        desired_count: Target number of molecules to generate (default: 100)
        
    Returns:
        List of validated unique molecules
    """
    if top_200_df.empty:
        print("Top 500 DataFrame is empty")
        return []
...
async def run_continuous_genetic_loop(state: Dict[str, Any]) -> None:
    ...
    1. Generate 100 unique molecules (using top 500 as seed pool)
    ...
    while not state['shutdown_event'].is_set():
        try:
            ...
            if top_200_df.empty:
                print("No top 500 molecules found, waiting...")
            ...
            if molecules_with_scores:
                ...
                # Add new molecules to top_200_df and keep top 500
                new_rows = []
