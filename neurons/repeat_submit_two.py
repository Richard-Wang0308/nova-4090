...
def get_top_molecule_from_db(db_path: str = None) -> Optional[str]:
    """
    Return the molecule_name with the 2nd highest score that is still available
    ...
    try:
        ...
        if len(rows) >= 2:
            molecule_name, score = rows[1]
            bt.logging.info(f"2nd top available molecule from DB: {molecule_name} (score: {score})")
            return molecule_name
        elif len(rows) == 1:
            molecule_name, score = rows[0]
            bt.logging.warning(
                f"Only 1 available molecule in DB, returning it instead: {molecule_name} (score: {score})"
            )
            return molecule_name
        else:
