...
def mark_molecule_unavailable(molecule_name: str, db_path: str = None) -> None:
    """
    Mark a molecule as unavailable (available = FALSE) in the scored_molecules table.
    Called after a molecule is successfully submitted.

    Args:
        molecule_name: The molecule name to mark as unavailable
        db_path: Path to the score_results database
    """
    if db_path is None:
        db_path = SCORE_RESULTS_DB
    ...
    try:
        ...
        if rows_affected > 0:
            bt.logging.info(f"✅ Marked molecule {molecule_name} as unavailable (available = FALSE)")
        else:
            bt.logging.warning(f"⚠️  Molecule {molecule_name} not found in scored_molecules table")
    except Exception as e:
        bt.logging.error(f"Error marking molecule {molecule_name} as unavailable: {e}")
            ...
            if commitment_status:
                try:
                    ...
                    if github_status:
                        ...
                        # ✅ Mark submitted molecule as unavailable in database
                        mark_molecule_unavailable(candidate_product)
                    else:
...
async def _find_best_unique_molecule(
    state: Dict[str, Any],
    all_scored_molecules: List[Dict[str, Any]]
) -> Optional[Dict[str, Any]]:
    """
    Find the best unique molecule from scored molecules list.
    Iterates through molecules sorted by score (descending) and returns
    the first one that passes the uniqueness check.

    Args:
        state: Miner state dictionary
        all_scored_molecules: List of scored molecule dicts

    Returns:
        Best unique molecule dict, or None if no unique molecule found
    """
    ...
    for mol in sorted_molecules:
        ...
        if is_unique:
            bt.logging.info(
                f"✅ Found best unique molecule: {mol_name} "
                f"(score: {mol.get('boltz_score', 'N/A')})"
            )
            return mol

    bt.logging.warning("⚠️  No unique molecules found in scored molecules list")
    return None


async def _try_submit_best_molecule(
    state: Dict[str, Any],
    all_scored_molecules: List[Dict[str, Any]],
    current_epoch: int
) -> bool:
    """
    Try to find the best unique molecule and submit it.
    ...
    Returns:
        True if submission was attempted (success or fail), False if no molecule to submit
    """
    last_submission_epoch = state.get('last_submission_epoch', -1)

    # Check if already submitted this epoch
    if last_submission_epoch == current_epoch:
        bt.logging.info(f"⏭️  Already submitted in epoch {current_epoch}")
        return True

    # Find best unique molecule
    best_molecule = await _find_best_unique_molecule(state, all_scored_molecules)
    ...
    best_score = best_molecule.get('boltz_score', float('-inf'))
    bt.logging.info(
        f"🏆 Best unique molecule: {best_molecule['name']} "
    ...
    try:
        ...
        bt.logging.info(
            f"✅ Submission successful for {best_molecule['name']} "
            f"(score: {best_score:.6f})!"
        )
        state['last_submission_epoch'] = current_epoch
        state['last_processed_epoch'] = current_epoch
                ...
                while not submitted:
                    ...
                    if blocks_remaining < 50:
                        if all_scored_molecules:
                            submitted = await _try_submit_best_molecule(
                                state, all_scored_molecules, current_epoch
                            )
                        else:
                            bt.logging.warning(
