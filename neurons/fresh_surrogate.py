"""
fresh_surrogate.py — DPEX-DJA + Boltz, single fixed reaction (no multi-rxn).

Same surrogate-guided pipeline as aggressive_surrogate.py, but search starts
FRESH (cold populations / empty top_pool). Historical sqlite + CSV are used
ONLY to train (and retrain) the surrogate — never to seed pop_A / pop_B /
top_pool / ComponentRanker.
...
Pipeline per iteration:
  ...
  4. Surrogate filter              → keeps top SURROGATE_KEEP_RATIO of fresh
                                      candidates once training data is large
                                      enough; else hard-cap at BOLTZ_BUDGET
  5. Boltz score                   → whatever survives step 4

Surrogate anchors: sqlite + CSV at start.
Surrogate retrain each iter: anchors + newly Boltz-scored live data.
Search start: cold random init (no warm-seed populations).
"""
...
RXN_CSV          = None   # data/rxn{rxn_id}.csv — surrogate training only (no search seed)
...
# Main optimization/progress signal: average score of top 50 molecules
# (independent of config num_molecules, which may be 1 for submission).
TOP_AVG_K = 50
...
def parse_args() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Fresh-start DPEX-DJA Miner — surrogate from history, search from cold"
    )
    parser.add_argument(
    ...
    logger.info(f"✅ RXN_CSV           = {RXN_CSV}  (surrogate training only)")
    logger.info(
        f"✅ Mode = FRESH SEARCH (no pool/pop seed from sqlite/csv) | "
...
class SurrogateModel:
    ...
    def _safe_fp(self, smiles: str) -> np.ndarray:
        ...
        Explicit `is None` check — avoids numpy ambiguous bool error.
        """
    ...
    def add_anchor_data(self, smiles_list: list, scores: list):
        ...
        # ── Filter non-finite scores first ─────────────────────────────
        ...
        # random from the middle (exclude top and bottom)
        middle     = list(set(range(n)) - top_idx - bottom_idx)
        rand_idx   = set(
            np.random.choice(
                middle, min(n_rand, len(middle)), replace=False
            ).tolist()
        ) if middle else set()
    ...
    def add_training_data(self, smiles_list: list, scores: list):
        ...
        # Balanced subsampling for live data
    ...
    try:
        ...
        if skipped_non_finite:
            logger.warning(
                ...
                f"non-finite score(s) (inf/-inf/nan) — not written to DB"
            )
...
def load_molecules_from_csv(
    csv_path: str, rxn_id: int, config: Dict[str, Any] = None
) -> pd.DataFrame:
    if config is None:
        config = {}
    ...
    try:
        ...
        banned_before = len(df)
        banned_atom_count = 0
        if config.get("banned_atom_types"):
            keep_mask = []
            for name, smiles in zip(df['molecule_name'], df['smiles']):
                ok, _ = validate_molecule_banned_atoms(name, smiles, config)
                if ok:
        ...
        logger.info(
            ...
            f"(banned atoms filtered: {banned_atom_count}/"
            f"{banned_before})"
        )
        return result
    except Exception as e:
    ...
    try:
        ...
        for mol_name, score in db_results:
            try:
                ...
                if config.get("banned_atom_types"):
                    ok, _ = validate_molecule_banned_atoms(
                        mol_name, smiles, config
                    )
                    if not ok:
                        banned_atom_count += 1
                        continue
        ...
        if non_finite:
            logger.warning(
                ...
                f"dropped {non_finite} non-finite (inf/-inf/nan) score rows"
            )
        logger.info(
            f"✅ DB [{os.path.basename(db_path)}]: "
            f"{len(result)} molecules loaded (fail={fail}, "
            f"non_finite={non_finite}, banned_atoms={banned_atom_count})"
        )
        return result
    except Exception as e:
...
def load_molecules_combined(
    ...
    logger.info(
        f"🔄 History load rxn={rxn_id}: "
        f"{os.path.basename(csv_path)} + {os.path.basename(db_path)}"
    )
    csv_df = load_molecules_from_csv(csv_path, rxn_id, config)
    db_df  = load_molecules_from_db(db_path,   rxn_id, config)
    if csv_df.empty and db_df.empty:
        logger.warning(f"⚠️  No historical data for rxn={rxn_id}")
        return pd.DataFrame(columns=["name", "smiles", "InChIKey", "score"])
    if csv_df.empty:
        return db_df
    if db_df.empty:
...
# ═══════════════════════════════════════════════════════════════════════════
# FRESH START (surrogate from history; search populations empty)
# ═══════════════════════════════════════════════════════════════════════════
...
def _log_pool_progress(
    ...
    Main progress metric is top-50 average score.
    ...
    avg_delta = (
        top50_avg - prev_top50_avg if prev_top50_avg is not None else None
    )
    max_delta = (
        pool_max - prev_max if prev_max is not None else None
    )

    lines = [
        "[PoolProgress] "
        + (f"iter={iteration}" if iteration > 0 else "fresh-start"),
    ]
    if mode:
    ...
    lines.append(
        f"  all-time top-{TOP_AVG_K} avg : {best_top50_avg_ever:.6f}"
    )
...
def fresh_start(
    ...
    """
    Train surrogate on historical sqlite+CSV, but do NOT seed search state.

    Returns (top_pool, all_pool, historical_names).
    historical_names is used later so disk reloads feed the surrogate only
    and never inject old molecules into the live search pools.
    """
    config = state['config']
    logger.info(
        f"\n{'='*60}\n"
        f"[FreshStart] rxn={RXN_ID} | SEARCH = cold (empty pools/pops)\n"
        f"[FreshStart] surrogate anchors: "
        f"rxn{RXN_ID}.csv + score_results_{RXN_ID}.sqlite\n"
        f"[FreshStart] surrogate activates once total training data "
        f">= {SURROGATE_MIN_TRAIN_SIZE}\n"
        f"{'='*60}"
    )

    # Keep search state empty — no pop_A / pop_B / top_pool / ranker seed.
    dpex.pop_A = []
    dpex.pop_B = []
    ...
    historical_names: set = set()

    if loaded_df.empty:
        logger.warning(
            "[FreshStart] No historical data — surrogate will cold-start "
            "from live Boltz scores only"
        )
    else:
        historical_names = set(loaded_df['name'].tolist())
        # Dedup only: avoid re-scoring known molecules. Do NOT seed pools.
        params.seen_molecules = set(historical_names)
        logger.info(
            f"[FreshStart] History: {len(loaded_df)} molecules → "
            f"surrogate anchors + seen dedup ONLY "
            f"(NOT seeding pop_A/pop_B/top_pool/ranker)"
        )
        ...
        logger.info(
            f"[FreshStart] Surrogate anchors from sqlite+csv: "
    ...
    if not training_df.empty:
        ...
        logger.info(
            f"[FreshStart] Surrogate anchors from rxn{RXN_ID}.csv "
    ...
    if surrogate.total_train_size < surrogate.min_train_size:
        logger.warning(
            f"[FreshStart] Insufficient historical data for surrogate "
    ...
    if surrogate.total_train_size >= surrogate.min_train_size:
        ...
        logger.info(
            f"[FreshStart] Surrogate pre-trained: "
    ...
    logger.info(
        f"\n[FreshStart] ✅ Complete!\n"
        f"  Search start : COLD (pop_A=0 pop_B=0 top_pool=0 all_pool=0)\n"
        f"  Ranker       : empty (no historical EMA)\n"
        f"  seen (dedup) : {len(params.seen_molecules)} historical names\n"
    ...
    _log_pool_progress(
        0, 0.0, 0.0, None,
        None, None, 0.0, 0.0, num_molecules,
        mode="FRESH-START",
    )

    return top_pool, all_pool, historical_names
...
async def find_solution(state: Dict[str, Any]) -> None:
    """
    Unlimited DPEX-DJA loop — fresh search start, history-trained surrogate.

    Per-iteration pipeline (order matters!):
      1. generate  n_base_samples * GENERATE_MULTIPLIER  (≈ 2000)
      2. resolve SMILES
      3. dedup seen        ← FIRST: remove already-scored molecules
      4. surrogate filter  ← SECOND: keep top SURROGATE_KEEP_RATIO
                              once enough training data exists
      5. Boltz score       ← receives whatever survives step 4

    Search populations start empty (cold). Historical sqlite/CSV train the
    surrogate only and are never injected into top_pool / pop_A / pop_B.
    """
    global molecule_manager
    ...
    top_pool, all_pool, historical_names = fresh_start(
    ...
    init_top50_avg, init_pool_max, _ = _top_pool_stats(top_pool, TOP_AVG_K)
    ...
    logger.info(
        f"🚀 FRESH DPEX-DJA loop | rxn={RXN_ID} | "
        f"A={len(molecule_manager.moles_A_id)} "
        f"B={len(molecule_manager.moles_B_id)} "
        f"C={len(molecule_manager.moles_C_id)} | "
        f"pipeline: generate {GENERATE_MULTIPLIER}x "
    ...
    try:
        while True:
            ...
            # ── Reload disk scores every 5 iters for SURROGATE only ───
            # Never merge historical molecules into all_pool / top_pool.
            if iteration % 5 == 0:
                loaded_df = load_molecules_combined(RXN_ID, config)
                if not loaded_df.empty and surrogate.enabled:
                    # Post-start scores only (exclude original history anchors)
                    new_for_surrogate = loaded_df[
                        ~loaded_df['name'].isin(historical_names)
                    ]
                    if not new_for_surrogate.empty:
                        params.seen_molecules = params.seen_molecules | set(
                            new_for_surrogate['name'].tolist()
                        )
                        valid_new = new_for_surrogate[
                            new_for_surrogate['score'].notna() &
                            new_for_surrogate['smiles'].notna()
                        ]
                        if not valid_new.empty:
                            ...
                            logger.info(
                                ...
                                f"post-start scores from disk "
                                f"(not injected into search pools; "
                                f"total_train_size="
                                f"{surrogate.total_train_size})"
                            )
                    # Track names we've already considered for disk→surrogate
                    historical_names |= set(loaded_df['name'].tolist())
            ...
            prev_top50_avg = (
                iter_prev_top50_avg if not top_pool.empty else None
            )
            ...
            # Keep enough diverse elites for a stable top-50 average
            # even when config num_molecules is 1.
            top_pool = select_tanimoto_diverse(
                all_pool.reset_index(drop=True),
                n=max(num_molecules, TOP_AVG_K) + 50,
            ...
            current_top50_avg = (
                top50_avg if not top_pool.empty else None
            )
            if (
            ...
            if not top_pool.empty:
                logger.info(
                    f"   🏆 Top-{TOP_AVG_K} avg: {top50_avg:.6f} | "
                    f"Best single: {best_name} (score={pool_max:.6f})"
                )
        ...
        if not top_pool.empty:
            ...
            logger.info(
                f"Final top-{TOP_AVG_K} avg: {final_top50_avg:.6f} | "
                f"best single: {final_best} (score={final_max:.6f}) | "
                f"all-time top-{TOP_AVG_K} avg={best_top50_avg_ever:.6f} | "
...
async def main():
    ...
    logger.info(
        f"🚀 Starting fresh_surrogate.py | rxn={rxn_id} | "
        f"surrogate DB=score_results_{rxn_id}.sqlite | "
        f"surrogate CSV=data/rxn{rxn_id}.csv | "
        f"search=FRESH (no warm-seed) | "
        f"progress=top-{TOP_AVG_K} average"
    )
