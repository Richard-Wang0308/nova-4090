...
async def startup_phase(state: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # Get top 500 molecules (already sorted by score in load_molecules_combined)
        ...
        bt.logging.info(f"✅ Loaded {len(molecules_df)} molecules from CSV and database (top 500: {len(top_200_df)})")
...
async def generate_unique_molecules_from_top200(
    ...
    if top_200_df.empty:
        bt.logging.warning("Top 500 DataFrame is empty")
        return []
    ...
    pool_sizes = [100, 150, 200, 350,500]
    ...
    while not state['shutdown_event'].is_set():
        try:
            ...
            if current_epoch != last_processed_epoch:
                ...
                # Get top 500 molecules (already sorted by score)
                top_200_df = molecules_df.head(500)
                ...
                bt.logging.info(f"✅ Reloaded {len(molecules_df)} molecules from CSV and database (top 500: {len(top_200_df)})")
