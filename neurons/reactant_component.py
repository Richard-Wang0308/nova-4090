"""
reactant_component.py — Exhaustive single-component search with surrogate
pre-filtering + Boltz scoring, for a fixed reaction (2 or 3 reactants).

Same pipeline as component_exhaust.py, except surrogate training data:
  - Only scored molecules whose fixed component position(s) match --fix/--value
  - No top-N / bottom-N size limit — use every matching scored molecule

2-reactant (A:B) usage:
    python reactant_component.py --rxn_id 2 --fix A --value 138879 --n 1000
    python reactant_component.py --rxn_id 2 --fix B --value 171540 --n 1000

3-reactant (A:B:C) usage:
    python reactant_component.py --rxn_id 3 --fix A,B --value 55,120 --n 150
    python reactant_component.py --rxn_id 3 --fix A,C --value 55,9   --n 150
    python reactant_component.py --rxn_id 3 --fix B,C --value 120,9 --n 150

Pipeline:
  1. Train surrogate on ALL scored molecules that match the fixed
     component(s) (rxn-specific, from score_results_{rxn_id}.sqlite)
...
def _name_matches_fixed(name: str, fixed: Dict[str, int]) -> bool:
    """
    Check whether molecule name matches fixed component id(s).

    Name formats:
      2-component: rxn:{rxn_id}:{A}:{B}
      3-component: rxn:{rxn_id}:{A}:{B}:{C}
    """
    if not fixed:
        return True
    parts = str(name).split(":")
    # parts[0]="rxn", parts[1]=rxn_id, parts[2]=A, parts[3]=B, [parts[4]=C]
    if len(parts) < 4:
        return False
    role_index = {"A": 2, "B": 3, "C": 4}
    for role, value in fixed.items():
        idx = role_index.get(role)
        if idx is None or idx >= len(parts):
            return False
        try:
            if int(parts[idx]) != int(value):
                return False
...
def load_all_scored(
    ...
    """
    Load scored rows for this reaction.

    If `fixed` is given, keep only molecules whose component position(s)
    match those fixed id(s) (e.g. fix A=138879 → only names with that A).
    """
    ...
    if fixed:
        mask = df["name"].apply(lambda n: _name_matches_fixed(n, fixed))
        df = df[mask]
...
# ═══════════════════════════════════════════════════════════════════════════
# Surrogate model (all scored molecules matching fixed component(s))
# ═══════════════════════════════════════════════════════════════════════════
class SurrogateModel:
    ...
    def train_from_db(
        ...
        df = load_all_scored(db_path, rxn_id, fixed=fixed)
        if df.empty:
            logger.warning(
                f"[SURROGATE] No scored data matching fixed={fixed} — cannot train"
            )
            return

        logger.info(
            f"[SURROGATE] Training set: {len(df)} scored molecules "
            f"matching fixed={fixed} (no size limit)"
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Exhaustive single-component search with surrogate + Boltz. "
            "Surrogate trains on ALL scored molecules matching --fix/--value "
            "(no top/bottom size limit)."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Component(s) to fix, comma-separated. "
             ...
             "Also filters surrogate training data to matching positions.",
    )
    parser.add_argument(
...
async def main():
    ...
    # ── Train surrogate on ALL scored mols matching fixed component(s) ──
    surrogate = SurrogateModel()
    surrogate.train_from_db(rxn_id, db_path, fixed=fixed)
