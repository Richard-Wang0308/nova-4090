...
# Bind at import time so a caller that imports this module as a library gets
...
async def refresh_historical_fingerprints(state: Dict[str, Any]) -> None:
    """Reload the cached historical fingerprints for the current target."""
    targets = state.get('current_challenge_targets') or []
    primary_target = targets[0] if targets else None

    if not primary_target:
        state['historical_df'] = None
        return

    state['historical_df'] = await asyncio.to_thread(
        load_historical_fingerprints, primary_target
    )

    historical_df = state['historical_df']
    logger.info(
        f"📚 Historical submissions for {primary_target}: "
        f"{0 if historical_df is None else len(historical_df)} "
        f"(reject Tanimoto >= {MAX_SIMILARITY_TO_HISTORICAL})"
    )


async def validate_molecule_historical_diversity(
    state: Dict[str, Any],
    molecule_name: str,
    smiles: str
) -> Tuple[bool, str]:
    """Validate molecule is not too similar to any historical submission."""
    if 'historical_df' not in state:
        await refresh_historical_fingerprints(state)

    historical_df = state.get('historical_df')
    if historical_df is None or historical_df.empty:
        return True, ""
    ...
    try:
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
            return False, "Cannot parse SMILES for historical similarity check"

        mol_fp = MORGAN_FP_GENERATOR.GetFingerprint(mol)
        if not is_diverse_enough(mol_fp, historical_df, MAX_SIMILARITY_TO_HISTORICAL):
            return False, (
                f"Too similar to a historical submission "
                f"(Tanimoto >= {MAX_SIMILARITY_TO_HISTORICAL})"
            )
        return True, ""
    except Exception as e:
        return False, f"Historical similarity check error: {str(e)}"
...
async def validate_molecule_complete(
    ...
    if not is_valid:
        errors.append(f"[HISTORICAL_SIM] {error_msg}")
        ...
        for mol in new_molecules:
            ...
            if not is_valid:
                for error in errors:
                    ...
                    elif "[HISTORICAL_SIM]" in error:
                        validation_stats['failed_historical_sim'] += 1
                    else:
...
async def run_generation_and_scoring_loop(state: Dict[str, Any]) -> None:
    ...
    try:
        while True:
            ...
            # ── Refresh historical submissions for the diversity check ──
            await refresh_historical_fingerprints(state)
...
async def main():
    ...
    # Track the validator's novelty threshold rather than a local constant.
    MAX_SIMILARITY_TO_HISTORICAL = novelty.config_threshold(config)
    logger.info(
        f"✅ max_similarity_to_historical = {MAX_SIMILARITY_TO_HISTORICAL} (from config)"
    )
