...
from utils.molecules import compute_fingerprint_entropy, is_boltz_safe_smiles, get_heavy_atom_count
...
class BoltzWrapper:
    ...
    def _preprocess_data_for_boltz(self, valid_molecules_by_uid: dict, score_dict: dict) -> None:
        ...
        try:
            for target, clip_interval in zip(self.subnet_config['small_molecule_target'], self.subnet_config['small_molecule_target_clip_interval']):
                print("------------------------------------------------")
                print(f"target: {target}, clip_interval: {clip_interval}")
    ...
    def score_molecules_target(self, valid_molecules_by_uid: dict, score_dict: dict, subnet_config: dict) -> None:
        # Preprocess data
