...
class BoltzWrapper:
    def __init__(self):
        ...
        # ====================================================================
        # LOAD TARGET PROTEIN FROM CONFIG (NO HuggingFace)
        # ====================================================================
        
        self.target_protein_id = self.config.get('target_protein_id')
        self.target_protein_sequence = self.config.get('target_protein_sequence')
        
        if not self.target_protein_id or not self.target_protein_sequence:
            raise ValueError("target_protein_id and target_protein_sequence must be in config.yaml")
        
        bt.logging.info(f"✅ Loaded target protein from config: {self.target_protein_id}")
        bt.logging.info(f"   Sequence length: {len(self.target_protein_sequence)} aa")
    ...
    def preprocess_data_for_boltz(self, valid_molecules_by_uid: dict, score_dict: dict, final_block_hash: str) -> None:
        ...
        self.protein_sequence = self.target_protein_sequence
        
        bt.logging.info(f"Using target protein sequence from config")
        bt.logging.info(f"   Protein ID: {self.target_protein_id}")
        bt.logging.info(f"   Sequence length: {len(self.protein_sequence)} aa")
