#!/usr/bin/env python3
"""
Same Murcko scaffold search against combinatorial_db/molecules.sqlite.

Finds molecules whose Bemis–Murcko core is exactly equal to the query scaffold.
Best for direct analog series with the same chemical core.

Example:
  .venv/bin/python scaffold_same_murcko.py --smiles "Nc1cn[nH]c(=O)c1" --role 2
  .venv/bin/python scaffold_same_murcko.py --mol-id 138879 --out same_murcko_hits.csv
"""
...
def resolve_query(
    ...
    if not scaffold:
        raise SystemExit(f"Could not extract Murcko scaffold from: {smiles}")
...
def main() -> int:
    p = argparse.ArgumentParser(description="Exact Murcko scaffold match over molecules.sqlite")
    ...
    p.add_argument("--out", type=Path, default=Path("same_murcko_hits.csv"))
    args = p.parse_args()
    ...
    print(f"Query Murcko scaffold: {q_scaffold}")
    ...
    with args.out.open("w", newline="") as f:
        w = csv.DictWriter(
            f, fieldnames=["morgan_tanimoto", "mol_id", "smiles", "role_mask", "murcko"]
        )
        w.writeheader()
        w.writerows(hits)
